...
public:
	...
	void computeMorphWeightsNormalised(const FVector2D& normal_pt)
	{
		...
		img_pt.X = std::max(std::min<float>((float)morph_data.morph_res - 1.0f, img_pt.X), 0.0f);
		img_pt.Y = std::max(std::min<float>((float)morph_data.morph_res - 1.0f, img_pt.Y), 0.0f);
