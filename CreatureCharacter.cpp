...
{
	UE_LOG(LogTemp, Warning, TEXT("ACreatureCharacter::Constructor Called()"));
	...
	ACreatureCharacter::LoadDataPacket(creature_json);
	ACreatureCharacter::LoadAnimation(creature_json, "default");
...
void ACreatureCharacter::InitStandardValues()
{
...
#if WITH_EDITOR
void ACreatureCharacter::PostEditChangeProperty(FPropertyChangedEvent & PropertyChangedEvent)
{
	bool retval = InitCreatureRender();
	if (retval)
	{
...
bool ACreatureCharacter::InitCreatureRender()
{
	/*
	if (!creature_mesh)
	{
	...
	*/
	...
	if (does_exist)
	{
		...
		ACreatureCharacter::LoadDataPacket(load_filename);
		...
		{
			ACreatureCharacter::LoadAnimation(load_filename, cur_name);
		...
		UE_LOG(LogTemp, Warning, TEXT("ACreatureCharacter::BeginPlay() - ERROR! Could not load creature file: %s"), *creature_filename);
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::Printf(TEXT("ACreatureCharacter::BeginPlay() - ERROR! Could not load creature file: %s"), *creature_filename));
	}
...
void ACreatureCharacter::FillBoneData()
{
...
void ACreatureCharacter::LoadDataPacket(const std::string& filename_in)
{
...
void ACreatureCharacter::LoadAnimation(const std::string& filename_in, const std::string& name_in)
{
...
void ACreatureCharacter::LoadCreature(const std::string& filename_in)
{
	if (!creature_mesh)
	{
		return;
...
bool ACreatureCharacter::AddLoadedAnimation(const std::string& filename_in, const std::string& name_in)
{
	auto cur_token = GetAnimationToken(filename_in, name_in);
	if (global_animations.count(cur_token) > 0)
	{
		...
		std::cout << "ERROR! ACreatureCharacter::AddLoadedAnimation() Animation with filename: " << filename_in << " and name: " << name_in << " not loaded!" << std::endl;
	}

	return false;
}

void ACreatureCharacter::SetBluePrintActiveAnimation(FString name_in)
{
...
void 
ACreatureCharacter::SetBluePrintAnimationLoop(bool flag_in)
{
	if (creature_manager) {
...
void 
ACreatureCharacter::SetBluePrintAnimationPlay(bool flag_in)
{
	should_play = flag_in;
	if (creature_manager) {
...
void 
ACreatureCharacter::SetBluePrintAnimationCustomTimeRange(FString name_in, int32 start_time, int32 end_time)
{
	auto cur_str = ConvertToString(name_in);
...
void ACreatureCharacter::SetActiveAnimation(const std::string& name_in)
{
	creature_manager->SetActiveAnimationName(name_in);
}

void ACreatureCharacter::SetBluePrintBlendActiveAnimation(FString name_in, float factor)
{
	auto cur_str = ConvertToString(name_in);
...
void 
ACreatureCharacter::SetAutoBlendActiveAnimation(const std::string& name_in, float factor)
{
	auto all_animations = creature_manager->GetAllAnimations();
...
FTransform
ACreatureCharacter::GetBluePrintBoneXform(FString name_in, bool world_transform)
{
...
bool
ACreatureCharacter::IsBluePrintBonesCollide(FVector test_point, float bone_size)
{
	if (bone_size <= 0)
	{
		bone_size = 1.0f;
...
float 
ACreatureCharacter::GetBluePrintAnimationFrame()
{
	return animation_frame;
}

void ACreatureCharacter::ParseEvents(float deltaTime)
{
...
void ACreatureCharacter::UpdateCreatureRender()
{
...
void ACreatureCharacter::GenerateTriangle(TArray<FProceduralMeshTriangle>& OutTriangles)
{
	FProceduralMeshTriangle triangle;
